...
| `auth-recovery` | Yes | Recovery email code and reset authorization | Public bootstrap plus optional user auth/2FA | Generic response timing, reset token, bearer for change flows | `recovery_request`, `recovery_verify`, 2FA | shared allowlist | service role, mail-transport secret | Enumeration/reset abuse | Acceptable with tests |
| `auth-reset-password` | Yes | OPAQUE password reset finish | Reset token | Active reset challenge lookup | `opaque_reset` | shared allowlist | service role, mail-transport secret | Unauthorized password reset | Acceptable with tests |

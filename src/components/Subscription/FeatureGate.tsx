...
export function FeatureGate({ feature, children, compact, featureLabel }: FeatureGateProps) {
    ...
    const { allowed, requiredTier, billingDisabled } = useFeatureGate(feature);

    // If billing is disabled (self-host) or feature is allowed, render children
    if (allowed || billingDisabled) {
        return <>{children}</>;
    }

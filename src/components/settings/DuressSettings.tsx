...
export function DuressSettings() {
    ...
    async function handleSetup() {
        ...
        try {
            ...
            // Pre-OpLog releases used to populate the user's `vault_items`
            // legacy table with default decoy entries (encrypted with the
            // duress key) right here. That path is intentionally removed:
            // on USK + OpLog vaults those direct inserts produce
            // "Tresor-Migration erforderlich" + IntegrityV2 `orphan_remote`,
            // because the rows are not part of any signed manifest. A
            // proper "decoy vault" needs its own signed OpLog scope; until
            // that is in place duress mode opens an empty decoy vault and
            // the user can add decoys themselves while signed in as duress.

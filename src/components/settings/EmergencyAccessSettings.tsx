...
export default function EmergencyAccessSettings({ bypassFeatureGate = false }: EmergencyAccessSettingsProps) {
    ...
    const canClaimPendingAccess = (access: EmergencyAccess): boolean => {
        if (access.status !== 'pending' || !access.requested_at) {
            return false;
        }

        const requestedAt = new Date(access.requested_at).getTime();
        if (!Number.isFinite(requestedAt)) {
            return false;
        }

        return Date.now() >= requestedAt + access.wait_days * 24 * 60 * 60 * 1000;
    };
                                    ...
                                    {grantors.map((item) => (
                                        <TableRow key={item.id}>
                                            ...
                                            <TableCell className="text-right space-x-2">
                                                ...
                                                {item.status === 'pending' && (
                                                    canClaimPendingAccess(item) ? (
                                                        <Button size="sm" onClick={() => emergencyAccessService.claimAccess(item.id).then(fetchData)}>
                                                            <Unlock className="w-4 h-4 mr-2" />
                                                            {t('emergency.claimAccess', 'Claim Access')}
                                                        </Button>
                                                    ) : (

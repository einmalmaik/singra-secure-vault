...
export default function GrantorVaultPage() {
    ...
    const fetchAndDecryptItems = async () => {
        ...
        try {
            ...
            try {
                cryptoKey = await importMasterKey(masterKeyBytes);
            } finally {
            ...
            const decryptedItems = await Promise.all(
                (vaultItems || []).map(async (item) => {
                    try {
                        const decryptedData = await decryptVaultItem(item.encrypted_data, cryptoKey, item.id);
                        return {
                            ...item,
                            decryptedData
                        } as VaultItem;
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col">
            ...
            {/* Main Header */}
            <header className="ms-glass-header sticky top-0 z-10 px-4 lg:px-6 py-4">

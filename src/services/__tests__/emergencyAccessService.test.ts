...
function createChainable(resolvedValue: unknown = { data: null, error: null }) {
  ...
  const methods = ["select", "insert", "update", "delete", "eq", "in", "is", "single", "maybeSingle", "limit", "order", "or"];
...
describe("getGrantors()", () => {
  it("returns grantors with profiles", async () => {
    ...
    const linkedChain = createChainable({ data: rows, error: null });
    const invitedChain = createChainable({ data: [], error: null });
    const profileChain = createChainable({ data: profiles, error: null });
    mockSupabase._setChains([linkedChain, invitedChain, profileChain]);

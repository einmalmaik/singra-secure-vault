 ...
 */
export async function attemptDuressUnlockOnly(input: {
    password: string;
    duressConfig: {
        enabled: boolean;
        salt: string | null;
...
}): Promise<{ matched: boolean; key: CryptoKey | null }> {
    try {
        const { password, duressConfig } = input;
        if (!duressConfig.enabled || !duressConfig.salt || !duressConfig.verifier) {
            await constantTimeDelay();
            return { matched: false, key: null };
        }

        const duressKey = await deriveKey(
            password,
            duressConfig.salt,
            duressConfig.kdfVersion ?? CURRENT_KDF_VERSION
        );

        const matched = await verifyKey(duressConfig.verifier, duressKey);
        await constantTimeDelay();

        if (matched) {
            return { matched: true, key: duressKey };
        }

        return { matched: false, key: null };
    } catch (err) {
        console.error('Duress-only unlock error:', err);
        await constantTimeDelay();
        return { matched: false, key: null };
    }
}
...
async function constantTimeDelay(): Promise<void> {
    const delay = randomBytes(1)[0] % 6; // 0-5ms

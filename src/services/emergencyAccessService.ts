...
export const emergencyAccessService = {
    ...
    async getGrantors() {
        ...
        const [linkedResult, invitedResult] = await Promise.all([
            supabase
                .from('emergency_access')
                .select('*')
            ...
            supabase
                .from('emergency_access')
                .select('*')
                .eq('trusted_email', normalizedEmail)
                .is('trusted_user_id', null)
                .order('created_at', { ascending: false }),
        ]);
        ...
        const rowsById = new Map<string, EmergencyAccess>();
        for (const row of [
            ...((linkedResult.data || []) as unknown as EmergencyAccess[]),
            ...((invitedResult.data || []) as unknown as EmergencyAccess[]),
        ]) {
            rowsById.set(row.id, row);
        }
        const rows = [...rowsById.values()].sort((left, right) => right.created_at.localeCompare(left.created_at));
    ...
    // Claim access after the configured wait period has elapsed (as trustee).
    async claimAccess(accessId: string) {
        const { data, error } = await supabase
            .from('emergency_access')
            .update({
                status: 'granted',

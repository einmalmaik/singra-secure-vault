-- Add the first server-authorized, read-only contract for central admin audit.
INSERT INTO public.team_permissions (permission_key, label, description, category)
VALUES (
  'admin.audit.read',
  'Read Admin Audit',
  'View constrained administrative audit events without Vault data or direct PII.',
  'admin'
)
...
VALUES ('admin'::public.app_role, 'admin.audit.read')
...
COMMENT ON FUNCTION public.list_admin_audit_events(UUID, INTEGER, TEXT, TEXT) IS
  'Service-role-only audit read boundary with database-level admin permission checks and constrained filters.';
